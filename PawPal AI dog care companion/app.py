...
genai.configure(api_key="AIzaSyBqvZ0rySj2bHTpt9z2CXTIc0n2ps0C5bk")


os.makedirs(app.config['UPLOAD_FOLDER'], exist_ok=True)

# --- Load Model ---
MODEL_PATH_PT = os.path.join(os.path.dirname(
    os.path.abspath(__file__)), 'kaggle_output1', 'best_model.pth')
NUM_CLASSES_PT = 6
CLASS_LABELS_PT = ['Dermatitis', 'Fungal_infections',
                   'Healthy', 'Hypersensitivity', 'demodicosis', 'ringworm']
model_pt = None
    ...
    try:
        ...
        for msg in conversation_history[-5:]:
            ...
            if role == "user":
                ...
                chat.send_message(
                    f"The AI assistant previously responded: {content}")
        ...
        if not text.startswith("•"):
            lines = text.split('\n')
            text = '\n'.join(f"• {line.strip()}" if line.strip(
            ) and not line.strip().startswith("•") else line for line in lines)
...
def chatbot():
    ...
    response = get_advanced_chatbot_response(
